...
// 四轮实验经验固化清单：
//   ① 恒等对账（第四炉装置 bug）：intended/observed 必须同一命名空间——open 写 expect，close 只做比较
//   ② 相位窗口（温区3 装置 bug）：所有写入 tmp+rename 原子，不留半写文件（半写=脏收养源）
//   ③ 空锚点目录容错（温区3 adopter bug）：recover 对空目录/缺文件全部降级容错
//   ④ 并发 rename 竞争（Witness 经验）：跨进程竞争败者静默
//   ⑤ Windows 路径清洗（第四炉装置 bug）：动作名非法字符替换
